...
function startServer() {
  ...
  // Get login-shell PATH (Electron from Finder has minimal PATH)
  let shellPath;
  try {
  ...
  // Find system node (Electron's node has different ABI for native modules)
  let systemNode;
  try {
  ...
  // Always use system Node (not Electron's Node) to avoid native module ABI mismatch
  const tsxCli = path.join(projectDir, "node_modules", "tsx", "dist", "cli.mjs");
  const spawnCmd = systemNode;
  const spawnArgs = [tsxCli, "server/index.ts"];
